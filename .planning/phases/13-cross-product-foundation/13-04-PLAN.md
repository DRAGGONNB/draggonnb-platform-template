...
__tests__/unit/modules/registry.test.ts
  ...
  <action>
NOTE on dependencies: This plan's `<context>` references `13-03-SUMMARY.md` for narrative context — it is OPTIONAL. Proceed without it if absent. The true hard dependencies are `lib/modules/types.ts` (TenantInputSpec, ApprovalActionSpec, TelegramCallbackSpec, BillingLineTypeSpec, ModuleManifest) and `lib/modules/registry.ts` (MODULE_REGISTRY, getManifestsForOrg, getAllApprovalActions, getAllTelegramCallbacks, getAllBillingLineTypes), both shipped in plan 13-03.
